...
# now mathc between data frames to get colors for locations
cl7 <- df_cll$colfcol_loc[match(poplocs2,df_cll$coll_loc2)]
# match back detailed location name
poploc3 <- df_cll$coll_loc[match(poploc,df_cll$coll_loc2)]
...
#_______________________________________________________________________________
# Make html table of samples
#_______________________________________________________________________________
# copy data frame
df_ihpt03 <- df_ihpt02
# split a column
df_ihpt03 <- tidyr::separate(df_ihpt03, pop, sep = "_", 
                             into = paste0(c("spcAccn","smplloc","smpl_year","NAbb"), 1:3)
                             , fill = "right")
# substitute in NCBI names
df_ihpt03$spcAccn1[grepl("NCBI",df_ihpt03$NAbb1)] <- gsub("Mnelei","",df_ihpt03$spcAccn1[grepl("NCBI",df_ihpt03$NAbb1)])
# copy data frame 
df_r04 <- df_r03
# match to get abbreviations for locations
df_ihpt03$locabbr <- df_clo03$locality8[match(df_ihpt03$smplloc2,df_clo03$locality4)]
# match to get abbreviations for thos that still are NA
df_ihpt03$locabbr[is.na(df_ihpt03$locabbr)] <- df_clo03$locality8[match(df_ihpt03$smplloc2[is.na(df_ihpt03$locabbr)],df_clo03$locality)]
# Replace in abbreviation for FBo
df_ihpt03$locabbr[df_ihpt03$smplloc2=="FynBogense"] <- "FBo"
# match to get sampling year
df_ihpt03$smpl_year4 <- df_lN02$smplyear[match(df_ihpt03$spcAccn1,df_lN02$accession_nmb)]
...
#match back lat lon positions to data frame
df_ihpt03$latlon4 <- df_clo03$latlonpos3[match(df_ihpt03$locabbr,df_clo03$locality8)]
#match back lat lon positions to data frame
df_r04$`Latitude Longitude for sample collection` <- df_ihpt03$latlon4[match(df_r04$`sample Number or NCBI Accession number`,df_ihpt03$spcAccn1)]
# match back to data frame
df_r04$`Year and month collected`[(df_r04$`Year and month collected`=="NA")]
miyeac <- df_r04$`sample Number or NCBI Accession number`[(df_r04$`Year and month collected`=="NA")]
df_r04$`Year and month collected`[(df_r04$`Year and month collected`=="NA")] <- df_ihpt03$smpl_year4[match(miyeac,df_ihpt03$spcAccn1)]
df_r04$`Year and month collected`[grepl("JX26",df_r04$`sample Number or NCBI Accession number`)] <- "2009"
df_r04$`Year and month collected`[grepl("GU",df_r04$`sample Number or NCBI Accession number`)] <- "2009"
